...
const FileUploadButton = ({ label, className, onUpload }: { label: string, className?: string, onUpload: (label: string, e: React.ChangeEvent<HTMLInputElement>) => void }) => (
  <label className={cn("cursor-pointer flex items-center gap-1 transition-colors text-slate-500 hover:text-purple-600", className)}>
    ...
    <form onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
      ...
      <div className="xl:col-span-8 space-y-8">
          ...
          <div className="dashboard-card space-y-6 flex flex-col h-full">
            ...
            <div className="flex items-center justify-between p-5 border border-slate-100 rounded-2xl bg-slate-50/50">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  <span className="font-bold text-slate-900 text-lg leading-tight">{formData.companyName}</span>
                  <span className="text-xs font-bold text-slate-400">Primary Branding</span>
                </div>
              </div>
              <FileUploadButton 
                label="Main Brand Logo" 
                onUpload={handleFileUpload} 
                className="text-xs px-4 py-2 border border-slate-200 rounded-xl font-bold bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50 shadow-sm" 
              />
            </div>
            ...
            <div className="space-y-4 flex-1">
              {[
                ...
                <div key={idx} className="flex items-center justify-between py-3 border-b border-slate-50 last:border-0 hover:bg-slate-50/30 rounded-lg px-2 -mx-2 transition-colors">
                  <div className="flex items-center gap-3">
                    {item.icon || <div className="w-4 h-4 rounded-full border-2 border-slate-300" />}
            ...
            <div className="space-y-2 pt-4 border-t border-slate-50">
              <label className="text-xs font-black text-slate-500 uppercase tracking-[0.1em]">
              ...
              <input 
                ...
                className="w-full px-5 py-3.5 rounded-2xl border border-slate-200 bg-white focus:outline-none focus:ring-4 focus:ring-purple-600/5 focus:border-purple-600 text-slate-900 font-bold transition-all"
            ...
            <div className="grid grid-cols-1 gap-5 flex-1">
              {[
                ...
                <div key={color.name} className="space-y-2">
                  <div className="flex justify-between items-center px-1">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">{color.label}</label>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative w-14 h-11 rounded-xl overflow-hidden shrink-0 border border-slate-200 shadow-sm cursor-pointer group">
                      <input 
                        type="color" 
                        name={color.name}
                    ...
                    <div className="relative flex-1">
                      <input 
                        ...
                        value={formData[color.name as keyof BrandingFormData] as string}
                        onChange={handleChange}
                        className="w-full uppercase px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-purple-600/10 focus:bg-white text-slate-700 font-mono text-sm font-bold transition-all"
                      />
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-2 pt-4 border-t border-slate-50 mt-auto">
                <div className="flex justify-between items-center px-1">
                  ...
                  <div className="flex items-center gap-1.5 bg-emerald-50 text-emerald-600 px-2.5 py-1 rounded-full text-[10px] font-black border border-emerald-100">
                    <Check className="w-3 h-3" /> WCAG PASS
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="relative w-14 h-11 rounded-xl overflow-hidden shrink-0 border border-slate-200 shadow-sm cursor-pointer">
                    <input 
                      type="color" 
                      name="textColor"
                  ...
                  <input 
                    ...
                    value={formData.textColor}
                    onChange={handleChange}
                    className="flex-1 uppercase px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-purple-600/10 focus:bg-white text-slate-700 font-mono text-sm font-bold transition-all"
                  />
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <div className="space-y-2" ref={fontRef}>
                ...
                <div className="relative">
                  <button
                    ...
                    className={cn(
                      "w-full px-5 py-3.5 rounded-2xl border bg-white focus:outline-none transition-all text-slate-900 flex items-center justify-between group",
                      isFontOpen ? "border-purple-600 ring-4 ring-purple-600/5 shadow-sm" : "border-slate-200 hover:border-slate-300"
                    )}
                  >
                    <span className="font-bold text-slate-800">{formData.fontFamily}</span>
                    <ChevronDown className={cn("w-5 h-5 text-slate-400 transition-transform duration-200", isFontOpen ? "rotate-180" : "group-hover:text-slate-600")} />
                  </button>
                  
                  <div className={cn(
                    "absolute z-20 w-full mt-2 bg-white border border-slate-100 rounded-3xl shadow-2xl shadow-slate-200/60 py-3 transition-all duration-200 transform origin-top left-0 overflow-hidden",
                    ...
                    {fontOptions.map((font) => (
                      <button
                        ...
                        className={cn(
                          ...
                          formData.fontFamily === font ? "bg-purple-50 text-purple-700 font-black" : "text-slate-700 hover:bg-slate-50 font-bold"
                        )}
                      >
                        {font}
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-xs font-black text-slate-500 uppercase tracking-wider">Heading Weight</label>
                  <div className="flex bg-slate-100/80 rounded-2xl p-1.5">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black text-slate-500 uppercase tracking-wider">Theme Mode</label>
                  <div className="flex bg-slate-100/80 rounded-2xl p-1.5 w-full">
          ...
          <div className="dashboard-card space-y-6 flex flex-col h-full">
              ...
              {[
                ...
                <div key={item.id} className="flex items-center justify-between p-4 rounded-2xl hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100 group">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] font-bold text-slate-400 group-hover:text-slate-500 transition-colors uppercase tracking-wider">{item.desc}</span>
                  </div>
                  <Switch 
            ...
            <div className="p-6 rounded-3xl bg-slate-900 text-white border border-slate-800 shadow-xl shadow-slate-200 relative overflow-hidden group">
               ...
               <div className="flex items-center justify-between mb-3 relative z-10">
                 <div className="flex items-center gap-3">
                    ...
                    <span className="font-black text-base">White Label Mode</span>
                 </div>
                 <Switch 
        ...
        <div className="dashboard-card border-slate-200 shadow-lg shadow-slate-100 flex flex-col md:flex-row items-center justify-between gap-6 p-6">
           <div className="flex flex-col gap-1 flex-1">
              <div className="flex items-center gap-3">
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Vanity URL</label>
                <div className="h-4 w-px bg-slate-100" />
                <button type="button" className="text-xs font-black text-purple-600 hover:text-purple-700 transition-colors">
                  DNS SETUP GUIDE
                </button>
              </div>
              <div className="flex gap-3 w-full max-w-md mt-1">
                <input 
                  ...
                  onChange={handleChange}
                  className="flex-1 px-4 py-2.5 rounded-xl border border-slate-100 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-purple-600/10 focus:bg-white text-slate-900 font-bold text-sm transition-all"
                />
              </div>
           </div>

           <div className="flex items-center gap-4 w-full md:w-auto">
              <div className="hidden sm:flex items-center gap-2 text-emerald-600 bg-emerald-50 px-4 py-2.5 rounded-2xl border border-emerald-100/50">
      ...
      <div className="xl:col-span-4 h-full">
        <div className="sticky top-28 space-y-6">
          <div className="dashboard-card space-y-4 bg-slate-100/50 border-slate-200/60 p-6 md:p-8 flex flex-col shadow-inner">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Real-time Preview</h2>
            ...
            <div 
              className="bg-white rounded-3xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.1)] overflow-hidden border border-slate-200/80 mx-auto w-full max-w-[340px] min-h-[550px] flex flex-col transition-all duration-500 relative flex-1 group/preview hover:scale-[1.02]"
              style={{ fontFamily: formData.fontFamily === 'Inter' ? undefined : formData.fontFamily }}
            >
               {/* Browser mock header */}
               <div className="bg-slate-50 h-10 flex items-center px-4 gap-2 border-b border-slate-200/50 shrink-0">
                 <div className="w-2.5 h-2.5 rounded-full bg-rose-400/50" />
                 <div className="w-2.5 h-2.5 rounded-full bg-amber-400/50" />
                 <div className="w-2.5 h-2.5 rounded-full bg-emerald-400/50" />
                 <div className="mx-auto w-24 h-2 bg-slate-200/70 rounded-full" />
               </div>
            ...
            <div className="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-sm mt-2">
               <div className="flex items-center gap-3">
                 <div className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" />
                 <p className="text-[10px] font-bold text-slate-500 leading-tight">
                   Preview represents a <span className="text-slate-800">standard dashboard dashboard</span> with your current brand configuration.
                 </p>
               </div>
            </div>
          </div>
